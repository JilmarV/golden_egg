...
def test_create_pay(_client):
    ...
    _client.post("/user/", json={
        "name": "User", "phone_number": "3133333333", "email": "SomeEmail@Mail.com",
        "username": "user", "password": "123", "address": "Somewhere",
        "enabled": True, "role_ids": [1]
    })
    _client.post("/order/", json={"totalPrice": 20100, "state": "pending", "user_id": 1})
    _client.post("/bill/", json={
        "totalprice": 5000, "paid": False, "order_id": 1,
        "issueDate": datetime.now().isoformat()
    })
    response = _client.post("/pay/", json={
        "amount_paid": 20100, "payment_method": "cash", "user_id": 1, "bill_id": 1
    })
...
def test_read_pays(_client):
    ...
    _client.post("/order/", json={"totalPrice": 20100, "state": "pending", "user_id": 1})
    _client.post("/bill/", json={
        "totalprice": 45000, "paid": False, "order_id": 1,
        "issueDate": datetime.now().isoformat()
    })
    _client.post("/pay/", json={
        "amount_paid": 20100, "payment_method": "cash", "user_id": 1, "bill_id": 1
    })
    response = _client.post("/pay/", json={
        "amount_paid": 2000, "payment_method": "cash", "user_id": 1, "bill_id": 1
    })
    assert response.status_code == 201
...
def test_read_pay(_client):
    ...
    create_response = _client.post("/pay/", json={
        "amount_paid": 20100, "payment_method": "cash", "user_id": 1, "bill_id": 1
    })
...
def test_update_pay(_client):
    ...
    created_pay = create_response.json()
    response = _client.put(f"/pay/{created_pay['id']}", json={
        "amount_paid": 30000, "payment_method": "credit_card", "user_id": 1, "bill_id": 1
    })
    assert response.status_code == 200
    data = response.json()
...
def test_total_earnings(_client):
    """Test retrieving total earnings."""
    _client.post("/role/", json={"name": "CUSTOMER"})
    _client.post("/user/", json={
        "name": "User", "phone_number": "3133333333", "email": "SomeEmail@Mail.com",
        "username": "user", "password": "123", "address": "Somewhere",
    ...
    assert response.status_code == 200
    assert response.json() == 20100

def test_total_earnings_month(_client):
    """Test retrieving total earnings by month."""
    _client.post("/role/", json={"name": "CUSTOMER"})
    _client.post("/user/", json={
        "name": "User", "phone_number": "3133333333", "email": "SomeEmail@Mail.com",
        "username": "user", "password": "123", "address": "Somewhere",
    ...
    assert response.status_code == 200
    assert response.json() == {'Del mes:': 5, 'En el año:': 2025, 'Total Pagado': 20100.0}
