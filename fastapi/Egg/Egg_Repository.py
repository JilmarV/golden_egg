...
import Egg_Schema
from fastapi.Egg import Egg_Model

# Create a new egg in the database
def create_egg(egg: Egg_Schema.EggCreate, db: Session = Depends(get_db)):
    db_egg = Egg_Model.Egg(**egg.dict())
    db.add(db_egg)
    db.commit()
    db.refresh(db_egg)
    return db_egg
...
def get_all_eggs(db: Session = Depends(get_db)):
    eggs = db.query(Egg_Model.Egg).all()
    return eggs

# Retrieves a specific egg by its ID
def get_egg_by_id(egg_id: int, db: Session = Depends(get_db)):
    egg = db.query(Egg_Model.Egg).filter(Egg_Model.Egg.id == egg_id).first()
...
def update_egg(egg_id: int, egg: Egg_Schema.EggCreate, db: Session = Depends(get_db)):
    db_egg = db.query(Egg_Model.Egg).filter(Egg_Model.Egg.id == egg_id).first()
